...
// Answer (not differentiating seat1, seat2)
/*
for (i = 0; i < N; i++)
{
	if (e1 <= s[i])
...
int max_s=0;
int seat1 = 0, seat2 = 0;
int seat1_index = 0, seat2_index = 0;
for (i = 0; i < s_len; i++)
{
	if (s[i] > max_s)
		max_s = s[i];
...
seat2 = e[seat1_index + 1];
seat2_index = seat1_index + 1;
printf("%d, %d", seat1_index, seat2_index);
printf("index\n");
while (seat1 <= max_s || seat2 <= max_s)
{
	int min_seat1 = 100000, min_seat2 = 100000;
	for (i = 0; i < e_len ; i++)
	{
		//printf("%d", s[i] - seat1);
		if (s[i] > seat1 && (s[i] - seat1 < min_seat1))
		{
			min_seat1 = s[i] - seat1;
			//printf("min seat1: %d", min_seat1);
			seat1_index = i;
		}
	}
	if(seat1 !=  e[seat1_index])
	{
		answer++;
		seat1 = e[seat1_index];
	}
	for (i = 0; i < e_len ; i++)
	{
		if (s[i] > seat2 && (s[i] - seat2 < min_seat2))
		{
			min_seat2 = s[i] - seat2;
			seat2_index = i;
		}
	}
	if (seat2 != e[seat2_index])
	{
		answer++;
		seat2 = e[seat2_index];
	}
